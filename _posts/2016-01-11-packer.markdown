...
to an endpoint) or [Vagrant](2016/01/28/vagrant) (to convert the image into a
valid Vagrant box).  
...
``` json
{
            ...
            "output_directory": "{% raw %}{{user `outputs_dir`}}{% endraw %}/{% raw %}{{user `vm_name`}}{% endraw %}_{% raw %}{{build_name}}{% endraw %}/"
        },
```
...
The most common use is the creation of a [Vagrant](2016/01/28/vagrant) box. But
    ...
    "post-processors": [
        {
            ...
            "compression_level": 9,
            "output": "{% raw %}{{user `outputs_dir`}}{% endraw %}/{% raw %}{{user `vm_name`}}{% endraw %}_{% raw %}{{.BuildName}}{% endraw %}.box"
        }
    ]
}
```
...
a point in detailing them here. Instead here is a general overview of
them:
