...
which will be approached in another tutorial where Ansible will also be
used.
...
Supported providers (ie: virtualization platforms) include: VirtualBox,
VMWare, HyperV, Docker, KVM, AWS and others.  
The provisioning (ie: installation and configuration of software into
the instantiated VM, or container in the case of Docker) can be done
using one or more of the supported configuration management tools: shell
scripts, Ansible, CFEngine, Chef, Docker, Puppet, Salt.  
...
The Vagrant boxes are created using Packer. And these consist on a tar
file which include the virtualization software VM files (eg: .ovf and
.vmdk) and a couple of Vagrant specific files.  
...
with Packer) and how to set up a development environment using a
Vagrantfile. It goes through the most commonly used Vagrantfile
settings.
...
The code is well commented and in the general term quite similar to the
code in the previous sections, so there isn't much to add here.
...
README for any relevant settings that you may need. However there are
two caveats to it:  
1) synced folders are supported via rsync (subject to its
limitations).  
...
-   Multi-machine environments  
    This will be a subject for another tutorial where Ansible will
    be used.
...
-   To run the latest version of Vagrant or any of its plugins from
    ...
    `vagrant_dev-wily64-atlas` recipe for more details, but it would go
    something like this:
...
[^2]: If you really need to have support for AWS credential files and
    ...
    take a look at the Vagrantfile for the AWS recipe at my github.
